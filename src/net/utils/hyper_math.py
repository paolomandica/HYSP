...
def contrastive_loss(x, pos, neg, tau, hyp_c):
    ...
    logits00 = dist_f(x, pos) / tau - eye_mask
    logits01 = dist_f(x, neg) / tau
...
def compute_delta(emb_path='results_posec3d.pkl'):
    emb = np.load(emb_path, allow_pickle=True)
    emb = torch.Tensor(np.stack(emb))
...
def dist_luca(x, y, *, c=1.0, keepdim=False):
    r"""
    Distance on the Poincare ball
    .. math::
        d_c(x, y) = \frac{2}{\sqrt{c}}\tanh^{-1}(\sqrt{c}\|(-x)\oplus_c y\|_2)
    ...
    """
    c = torch.as_tensor(c).type_as(x)
    return _dist_luca(x, y, c, keepdim=keepdim)


def _dist_luca(x, y, c, keepdim: bool = False):
    sqrt_c = c ** 0.5
    dist_c = artanh(sqrt_c * _mobius_add(-x, y, c).norm(dim=-1, p=2, keepdim=keepdim))
    return dist_c * 2 / (sqrt_c * torch.linalg.norm(x, dim=-1))